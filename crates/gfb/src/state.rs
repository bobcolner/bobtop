//! Persistent UI state for gfb.
//!
//! Currently just `last_cwd`: the directory the user was browsing
//! when they last quit. Loaded by `cli::run` when no path arg is
//! provided so reopening the browser (`gtop fb`, or `b` from the
//! main UI on a non-process row) resumes where you left off.
//!
//! Stored at `$XDG_STATE_HOME/gfb/state.toml` (default
//! `~/.local/state/gfb/state.toml`). Best-effort — read/write
