...
pub fn run(args: &[String]) -> i32 {
    if args.is_empty() {
        eprintln!("usage: gtop agent <subcommand>");
        eprintln!();
    ...
    let request = match request {
        Ok(r) => r,
        Err(msg) => {
            eprintln!("gtop agent {}: {msg}", args[0]);
            return 2;
        }
    };
    send(request)
...
fn print_help() {
    println!("gtop agent — query the running daemon over its Unix socket");
    println!();
    println!("subcommands:");
    ...
    println!();
    println!("examples:");
    println!("  gtop agent snapshot");
    println!("  gtop agent top --by cpu --n 5");
    println!("  gtop agent top --by mem --group exec --match '*chrome*' --match 're:^pg_'");
    println!("  gtop agent top --by cpu --group cgroup");
    println!("  gtop agent top --by cpu --group tree --n 10");
    println!("  gtop agent window --metric cpu --window 5m");
    println!("  gtop agent peak --metric net.tx --window 1m");
    println!("  gtop agent summary --match 'python*'");
    println!("  gtop agent pid_inspect --pid 1234");
    println!("  gtop agent responsible_for --metric cpu --at 30s");
}

/// Send a request through the engine's auto-spawn-aware client. Auto-
/// spawn forks `gtop --daemon` if no daemon is currently listening.
fn send(request: serde_json::Value) -> i32 {
    let path = socket_path_buf();
    let exe = match std::env::current_exe() {
        Ok(p) => p,
        Err(e) => {
            eprintln!("gtop agent: cannot locate own binary ({e})");
            return 4;
        }
    };
    match auto_query(&path, &exe, &["--daemon"], &request) {
        ...
        Err(msg) => {
            eprintln!(
                "gtop agent: {msg}. Try `gtop --daemon &` or run a TUI."
            );
            3
        }
    }
